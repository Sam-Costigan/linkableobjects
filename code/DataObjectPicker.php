...
class DataObjectPicker extends TextField
{
    ...
    protected function classToPick()
    {
        if (empty($this->config['classToPick'])) {
            ...
            if (empty($relationNames[$relationName])) {
                trigger_error("Can't figure out which class to search in. Please setup 'classToPick' using DataObjectPicker::setConfig().");
            }
        ...
        if (empty($this->config['summaryFields'])) {
            if (Config::inst()->get($this->classToPick(), 'summary_fields')) {
                ...
                foreach (Config::inst()->get($this->classToPick(), 'summary_fields') as $key => $val) {
                    ...
                    if (strpos($sf, '.') === false) {
                        $this->config['summaryFields'][] = trim($sf, '"');
                    }
                }
            } else {
                $this->config['summaryFields'] = array();
                foreach (Config::inst()->get($this->classToPick(), 'db') as $sf => $void) {
                    if (count($this->config['classToPick']) < 3) {
                        $this->config['summaryFields'][] = $sf;
                    }
                }
            }
        }
